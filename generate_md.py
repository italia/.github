...
def load_groups_and_repos():
    ...
    for organization in ORGANIZATIONS:
        inactive_repos.extend(
            get_inactive_repos(gh_connection, INACTIVE_DAYS_TRESHOLD, organization)
        )
        ...
        if repo.html_url not in inactive_repos_urls:
            ...
            if not repo.html_url in repositories:
                ...
                if repositories[repo.html_url]:
                    group = [
                        gr for gr in groups if gr["id"] == repositories[repo.html_url]
                    ][0]
                    ## Init group
                    ...
                    group["repos"].append(
                        {
                            ...
                            "organization": repo.html_url.replace(
                                "https://github.com/", ""
                            ).split("/")[0],
                        }
                    )
